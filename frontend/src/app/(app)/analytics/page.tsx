...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    Promise.all([getAnalytics(), getPostAnalytics()])
      .then(([overview, published]) => {
  ...
  return (
    <AppShell title="Analytics">
      <div className="space-y-6">
          ...
          <Card>
            <CardContent className="flex gap-3 p-5">
              ...
              <div>
                ...
                <p className="text-xl font-semibold">{`${analytics?.engagement_rate ?? 0}%`}</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Insight de IA</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-muted-foreground">{analytics?.ai_insight ?? "Cargando métricas reales..."}</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader>
            <CardTitle>Publicaciones publicadas</CardTitle>
          </CardHeader>
              ...
              posts.map((post) => (
                <div key={post.id} className="rounded-lg border p-4">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    {post.impressions ?? 0} impresiones · {post.likes ?? 0} likes · {post.comments ?? 0} comentarios
                  </p>
                </div>
              ))
            ) : (
              <p className="text-muted-foreground">Aún no hay métricas de publicaciones publicadas.</p>
            )}
          </CardContent>
        </Card>
      </div>
